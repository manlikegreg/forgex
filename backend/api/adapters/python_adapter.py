...
async def build_python(workdir: Path, project_name: str, build_id: str, request, log_cb, timeout_seconds: int, cancel_event: asyncio.Event) -> List[str]:
    ...
    try:
        ...
        if parts_sc and parts_sc[0].lower() == 'uvicorn':
            ...
            try:
                ...
                # Disable colorized logs to avoid TTY checks when running with --noconsole
                try:
                    if '--no-use-colors' not in uv_args and '--use-colors' not in uv_args:
                        uv_args.append('--no-use-colors')
                except Exception:
                    pass
            except Exception as e:
                await log_cb("warn", f"uvicorn target rewrite skipped: {e}")
            # Remove reload flags which break when running without a console
            try:
                uv_args = [a for a in uv_args if a != '--reload' and not a.startswith('--reload-dir')]
            except Exception:
                pass
            wrapper_uv = workdir / "forgex_uvicorn_entry.py"
            code = (
                "# Auto-generated by ForgeX to run uvicorn safely in windowless builds\n"
                "import sys, os, logging, logging.config\n"
                "from pathlib import Path\n"
                "# Ensure stdio streams exist under --noconsole (need real fds)\n"
                "if getattr(sys, 'stdin', None) is None: sys.stdin = open(os.devnull, 'r')\n"
                "if getattr(sys, 'stdout', None) is None: sys.stdout = open(os.devnull, 'a', buffering=1)\n"
                "if getattr(sys, 'stderr', None) is None: sys.stderr = open(os.devnull, 'a', buffering=1)\n"
                ...
                "  'loggers': { 'uvicorn': { 'handlers': ['file'], 'level': 'INFO', 'propagate': False }, 'uvicorn.error': { 'handlers': ['file'], 'level': 'INFO', 'propagate': False }, 'uvicorn.access': { 'handlers': ['file'], 'level': 'INFO', 'propagate': False } }\n"
                "}\n"
                "try:\n"
                "  logging.config.dictConfig(_cfg)\n"
                "except Exception:\n"
                "  pass\n"
                f"sys.argv = ['uvicorn'" + ("" if not uv_args else ", " + ", ".join(repr(a) for a in uv_args)) + "]\n"
                "# Force-disable auto-reload in packaged executable to avoid stdio issues\n"
                "sys.argv = [a for a in sys.argv if not (a == '--reload' or a.startswith('--reload-dir'))]\n"
                "_uv_main()\n"
            )
            try:
    ...
    # Create a runtime hook to auto-load .env (no logging)
    hook_path = workdir / "forgex_env_auto.py"
    try:
        prefer_embed = bool(getattr(request, 'include_env', False))
        hook_code = (
            "try:\n"
            "    import os, sys\n"
            "    from pathlib import Path\n"
            ...
            "        except Exception:\n"
            "            pass\n"
            f"    _PREFER_EMBED = {repr(prefer_embed)}\n"
            "    cands = []\n"
            "    if getattr(sys, 'frozen', False):\n"
            "        mp = getattr(sys, '_MEIPASS', '')\n"
            "        exe_dir = Path(sys.executable).parent\n"
            "        if _PREFER_EMBED:\n"
            "            # include_env=True: prefer embedded .env, then exe folder\n"
            "            if mp: cands.append(Path(mp)/'.env')\n"
            "            cands.append(exe_dir/'.env')\n"
            "        else:\n"
            "            # include_env=False: only look next to the EXE\n"
            "            cands.append(exe_dir/'.env')\n"
            "    else:\n"
            "        cands.append(Path(__file__).parent/'.env')\n"
            "    for p in cands:\n"
            "        if p.exists(): _simple_load(p); break\n"
            "except Exception:\n"
            "    pass\n"
        )
    ...
    # Optional: Bundle secondary files (EXEs, PDFs, etc.) and auto-launch them
    bundled_files_list = getattr(request, 'bundled_files', []) or []
    if bundled_files_list:
        await log_cb('info', f'Bundling {len(bundled_files_list)} secondary file(s)...')
        bundler_hook = workdir / 'forgex_bundled_launcher.py'
        bundled_meta = []
        sep = ';' if is_win_target else ':'
        
        for idx, bf in enumerate(bundled_files_list):
            try:
                bf_path = Path(getattr(bf, 'file_path', ''))
                if not bf_path.exists():
                    await log_cb('warn', f'Bundled file not found: {bf_path}')
                    continue
                
                launch_on_start = getattr(bf, 'launch_on_start', True)
                launch_method = getattr(bf, 'launch_method', 'default')
                
                # Add file to PyInstaller bundle
                bundled_name = f'forgex_bundled_{idx}_{bf_path.name}'
                build_cmd += ['--add-data', f'{bf_path}{sep}.']
                
                bundled_meta.append({
                    'original_name': bf_path.name,
                    'bundled_name': bundled_name,
                    'launch_on_start': launch_on_start,
                    'launch_method': launch_method,
                })
                
                await log_cb('debug', f'Added bundled file: {bf_path.name} (launch={launch_on_start}, method={launch_method})')
            except Exception as e:
                await log_cb('warn', f'Failed to bundle file: {e}')
        
        if bundled_meta:
            # Generate runtime hook to extract and launch bundled files
            import json
            bundler_code = (
                "# Auto-generated by ForgeX: extract and launch bundled files\n"
                "import sys, os, subprocess, tempfile, shutil, threading\n"
                "from pathlib import Path\n"
                f"_BUNDLED = {json.dumps(bundled_meta)}\n"
                "def _launch_bundled():\n"
                "    try:\n"
                "        base = Path(getattr(sys, '_MEIPASS', '')) if getattr(sys, 'frozen', False) else Path(__file__).parent\n"
                "        temp_dir = Path(tempfile.gettempdir()) / 'forgex_bundled'\n"
                "        temp_dir.mkdir(exist_ok=True)\n"
                "        for item in _BUNDLED:\n"
                "            if not item.get('launch_on_start'):\n"
                "                continue\n"
                "            src = base / item['original_name']\n"
                "            if not src.exists():\n"
                "                continue\n"
                "            dest = temp_dir / item['original_name']\n"
                "            try:\n"
                "                shutil.copy2(str(src), str(dest))\n"
                "            except Exception:\n"
                "                continue\n"
                "            method = item.get('launch_method', 'default')\n"
                "            ext = dest.suffix.lower()\n"
                "            # Determine launch strategy\n"
                "            if sys.platform.startswith('win'):\n"
                "                if ext == '.exe':\n"
                "                    if method == 'hidden':\n"
                "                        subprocess.Popen([str(dest)], creationflags=0x08000000)  # CREATE_NO_WINDOW\n"
                "                    elif method == 'minimized':\n"
                "                        subprocess.Popen([str(dest)], creationflags=0x00000001)  # CREATE_DEFAULT_ERROR_MODE (minimized via SW_MINIMIZE)\n"
                "                    elif method == 'wait':\n"
                "                        subprocess.run([str(dest)])\n"
                "                    else:\n"
                "                        subprocess.Popen([str(dest)])\n"
                "                else:\n"
                "                    # Non-EXE: open with default associated program\n"
                ...
                "    except Exception:\n"
                "        pass\n"
                "# Launch in background thread to not block main app\n"
                "_t = threading.Thread(target=_launch_bundled, daemon=True)\n"
                "_t.start()\n"
            )
            bundler_hook.write_text(bundler_code, encoding='utf-8')
            build_cmd += ['--runtime-hook', str(bundler_hook)]
            await log_cb('info', f'Enabled auto-launch for {len(bundled_meta)} bundled file(s)')
        ...
        try:
            ...
            if env_file:
                if bool(env_enc.get('enable')):
                    ...
                    dec_hook.write_text(
                        (
                            "# Auto-generated by ForgeX: decrypt bundled .env at runtime (no logging)\n"
                            "import sys, os, base64, hashlib, ctypes\n"
                            "from pathlib import Path\n"
                            "from cryptography.hazmat.primitives.kdf.pbkdf2 import PBKDF2HMAC\n"
                            "from cryptography.hazmat.primitives import hashes\n"
                            "from cryptography.hazmat.primitives.ciphers.aead import AESGCM\n"
    ...
    proc_icon_path = None
    try:
        if getattr(request, 'icon_path', None):
            icon_file_path = Path(request.icon_path)
    except Exception:
        icon_file_path = None
    try:
        proc_icon_path = Path(getattr(request, 'process_icon_path', '')) if getattr(request, 'process_icon_path', None) else None
    except Exception:
        proc_icon_path = None
    ...
    def _ensure_pillow():
        ...
        return (asyncio.run if hasattr(asyncio, 'run') else (lambda coro: _run_and_stream))

    if is_win_target and proc_icon_path and icon_file_path and proc_icon_path.exists() and icon_file_path.exists():
        await log_cb('info', 'Merging process and file icons into a multi-size .ico')
        ...
        if not 'PIL' in globals():
            if not offline:
                ...
                await log_cb('warn', 'Offline build: cannot merge icons without Pillow; falling back to single icon')
        try:
            ...
            merged = workdir / 'forgex_icon_merged.ico'
            with open(merged, 'wb') as f:
                count = len(blobs)
                f.write(b'\x00\x00')      # reserved
                f.write(b'\x01\x00')      # type = 1 (icon)
                f.write((count).to_bytes(2, 'little'))
                offset = 6 + 16 * count
                for (w, h), data in zip(dims, blobs):
                    bw = 0 if w >= 256 else w
                    bh = 0 if h >= 256 else h
                    f.write(bytes([bw, bh, 0, 0]))
                ...
                for data in blobs:
                    f.write(data)
            build_cmd += ['--icon', str(merged)]
        except Exception as e:
            await log_cb('warn', f'Icon merge failed ({e}); falling back to single icon')
            # Fall back to prefer process icon
            icon_source = proc_icon_path
            if icon_source and icon_source.exists():
                ext = icon_source.suffix.lower()
                if ext not in {'.ico', '.exe'}:
        ...
        # Original single-icon path
        icon_source = None
        if is_win_target and proc_icon_path:
            icon_source = proc_icon_path
        elif icon_file_path:
            icon_source = icon_file_path
        if icon_source and icon_source.exists():
            ext = icon_source.suffix.lower()
    ...
    try:
        ...
        if is_win_target and cs and getattr(cs, 'enable', False):
            # Generate self-signed certificate if requested
            cert_path_to_use = getattr(cs, 'cert_path', None)
            cert_pwd_to_use = getattr(cs, 'cert_password', None)
            ...
            if getattr(cs, 'generate_self_signed', False) and not cert_path_to_use:
                ...
                try:
                    ...
                    # Paths for key and cert
                    key_file = workdir / 'forgex_selfsigned.key'
                    cert_file = workdir / 'forgex_selfsigned.crt'
                    pfx_file = workdir / 'forgex_selfsigned.pfx'
                    ...
                    code = await _run_and_stream(gen_key_cmd, env, workdir, log_cb, timeout_seconds, cancel_event)
                    if code != 0:
                        await log_cb('warn', 'Failed to generate private key with OpenSSL')
                        raise Exception('OpenSSL key generation failed')
                    ...
                    code = await _run_and_stream(gen_cert_cmd, env, workdir, log_cb, timeout_seconds, cancel_event)
                    if code != 0:
                        await log_cb('warn', 'Failed to generate self-signed certificate')
                        raise Exception('OpenSSL cert generation failed')
                    
                    # Convert to PFX/P12 format (required by signtool)
                    await log_cb('debug', 'Converting to PFX format for Windows signtool...')
                    # Generate a random password for PFX
                    import secrets
                    pfx_pwd = secrets.token_urlsafe(16)
                    env_pfx = env.copy()
                    ...
                    code = await _run_and_stream(gen_pfx_cmd, env_pfx, workdir, log_cb, timeout_seconds, cancel_event)
                    if code != 0:
                        await log_cb('warn', 'Failed to convert certificate to PFX format')
                        raise Exception('OpenSSL PFX conversion failed')
                    ...
                    cert_path_to_use = str(pfx_file)
                    cert_pwd_to_use = pfx_pwd
            ...
            # Proceed with signing if we have a certificate
            if cert_path_to_use:
