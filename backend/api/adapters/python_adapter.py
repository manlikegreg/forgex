...
async def build_python(workdir: Path, project_name: str, build_id: str, request, log_cb, timeout_seconds: int, cancel_event: asyncio.Event) -> List[str]:
    ...
    try:
        ...
        if parts_sc and parts_sc[0].lower() == 'uvicorn':
            ...
            code = (
                "# Auto-generated by ForgeX to run uvicorn with file-only logging\n"
                "import sys, os, logging, logging.config, json\n"
                "from pathlib import Path\n"
                "# Ensure stdio streams exist under --noconsole\n"
                "class _FGXNullIO:\n"
                "  def write(self, *a, **k): pass\n"
                "  def flush(self): pass\n"
                "  def isatty(self): return False\n"
                "if getattr(sys, 'stdout', None) is None: sys.stdout = _FGXNullIO()\n"
                "if getattr(sys, 'stderr', None) is None: sys.stderr = _FGXNullIO()\n"
