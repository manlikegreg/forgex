...
async def build_python(workdir: Path, project_name: str, build_id: str, request, log_cb, timeout_seconds: int, cancel_event: asyncio.Event) -> List[str]:
    ...
    try:
        ...
        if parts_sc and parts_sc[0].lower() == 'uvicorn':
            uv_args = parts_sc[1:]
            # Try to extract module from first arg like "main:app" to ensure PyInstaller includes it
            try:
                if uv_args:
                    uv_mod = (uv_args[0] or '').split(':')[0]
                    if uv_mod:
                        build_cmd += ["--hidden-import", uv_mod]
                        await log_cb("debug", f"Added hidden-import for uvicorn module: {uv_mod}")
            except Exception:
                pass
            # If entry points to a subfolder (e.g., test/main.py) but user passed 'main:app', rewrite to 'test.main:app'
            try:
                if uv_args and ':' in (uv_args[0] or '') and entry:
                    import pathlib as _pl
                    ent = str(entry)
                    if ('/' in ent or '\\' in ent):
                        dotted = _pl.Path(ent).with_suffix('').as_posix().replace('/', '.')
                        app_attr = (uv_args[0] or '').split(':', 1)[1] if ':' in (uv_args[0] or '') else ''
                        uv_args[0] = f"{dotted}:{app_attr or 'app'}"
                        await log_cb("debug", f"Rewrote uvicorn target to {uv_args[0]}")
            except Exception:
                pass
            wrapper_uv = workdir / "forgex_uvicorn_entry.py"
            code = (
                "# Auto-generated by ForgeX to run uvicorn with provided CLI args\n"
                "import sys\n"
    ...
    try:
        if is_win_target and getattr(request, 'win_autostart', False) and getattr(request, 'output_type', '') == 'exe':
            ...
            win_hook_code = (
                ...
                "        name = os.path.splitext(os.path.basename(sys.executable))[0] + ' (ForgeX)'\n"
                "        exists = False\n"
                "        try:\n"
